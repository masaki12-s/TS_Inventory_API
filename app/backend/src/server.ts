...
connection.then((conn: any) => {
    ...
    //     res.setHeader('Location', `/stock/${stocks.name}`);
    //     if (!stocks.name) {
    ...
    **/
    app.get('/stocks/:name?', (req: express.Request, res: express.Response) => {
        ...
        let result: Stock[] = [];
        if (name) {
            conn.query('SELECT name, amount FROM stock WHERE name = ?', [name], (_error: any, result: any) => {
                if (result.length === 0) {
            ...
            conn.query('SELECT name, amount FROM stock WHERE amount > 0 ORDER BY name ASC', (error: any, result: any) => {
                // 形式を変換
                // 例: {"apple": 1, "banana": 2, "orange": 0}
                if (error) {
                    console.log(error);
